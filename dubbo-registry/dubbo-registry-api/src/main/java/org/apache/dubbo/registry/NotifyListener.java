...
public interface NotifyListener {
     ...
     *     总是以服务接口和数据类型为维度全量通知，即不会通知一个服务的同类型的的部分数据，用户不需要对比上一次的通知结果
     ...
     *     中途变更时，允许不同类型的数据分开通知，比如 providers,consumers, routers, overrides 允许只通知其中一种类型，但该类型的数据必须时全量的，而不是增量的
     * 4. If a data type is empty, need to notify a empty protocol with category parameter identification of url data.<br>
     *     如果一种类型的数据为空，需通知一个 empty 协议并带 category 参数的标示性 URL 数据
     ...
     *     通知者(即注册中心实现)需保证通知的循序，比如，单线程推送，队列串行话，带版本对比
     *
