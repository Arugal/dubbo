...
public class DubboBeanDefinitionParser implements BeanDefinitionParser {
    ...
    private static BeanDefinition parse(Element element, ParserContext parserContext, Class<?> beanClass, boolean required) {
        ...
        // 引用缺省是延迟初始化，只有引用被注入到其他 Bean,或被 getBean() 获取，才会初始化，如需要饥饿加载，
        // 既没有引用也立即生成代理类，可以配置: <dubbo:reference ... init="true" />
        ...
        if (ProtocolConfig.class.equals(beanClass)) {
            ...
            // 处理 class 属性，例如<dubbo:service id="sa" interface="com.alibaba.dubbo.demo.DemoService" class="com.alibaba.dubbo.demp.provider.DemoServiceImpl" >
        ...
        } else if (ProviderConfig.class.equals(beanClass)) { // 解析 <dubbo:provider /> 的内嵌子元素 <dubbo:servier />
        ...
        // 循环 Bean 对象的 setting 方法，将属性添加到 Bean 对象的属性复制
        for (Method setter : beanClass.getMethods()) {
            ...
            if (name.length() > 3 && name.startsWith("set")
                ...
                // getter is null or not public or getter.return.type != setter.param.type
                if (getter == null
                ...
                if ("parameters".equals(property)) { // 解析 <dubbo:parameters />
                    ...
                    if (value != null) {
                        ...
                        if (value.length() > 0) {
                            ...
                            if ("registry".equals(property) && RegistryConfig.NO_AVAILABLE.equalsIgnoreCase(value)) {
                            ...
                            } else if ("provider".equals(property) && value.indexOf(',') != -1) { // 多服务提供着的情况
                                ...
                                if (isPrimitive(type)) {
                                    ...
                                    // 创建 RuntinmeBeanReference 指像回调对象
                                    reference = new RuntimeBeanReference(returnRef);
                                    // 设置 onrentureMethod 到 BeanDefintion 的属性值
                                    ...
                                    // 创建 onthrowMethod 到BeanDefiniton 的属性值
                                    ...
                                    // 创建 RuntimeBeanReference， 指向 Service 的 Bean
     ...
     * @param required 是否需要 Bean 的 id 的类
     * @param tag 标签
    ...
    /**
     *  解析 <dubbo:service class="" /> 情况下内涵的 <property /> 的赋值
     * @param nodeList 子元素组
     * @param beanDefinition Bean 定义对象
     */
                ...
                if (node instanceof Element) {
                    if ("property".equals(node.getNodeName())
                        ...
                        if (name != null && name.length() > 0) {
                            ...
                            } else if (ref != null && ref.length() > 0) { // ref
